...
create table public.subscriptions (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id), order_id uuid not null references public.orders(id), provider_customer_ref text, provider_mandate_ref text, state text not null, renews_at timestamptz, created_at timestamptz not null default now());
create table public.sites (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id), order_id uuid not null references public.orders(id), url text not null, access_state text not null default 'not_requested', created_at timestamptz not null default now());
create table public.cases (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id), order_id uuid not null references public.orders(id), site_id uuid not null references public.sites(id), status public.case_status not null default 'awaiting_payment', assigned_staff_id uuid references auth.users(id), payment_verified_at timestamptz, access_usable_at timestamptz, response_deadline_at timestamptz, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), check (response_deadline_at is null or (payment_verified_at is not null and access_usable_at is not null)));
create table public.case_events (id uuid primary key default gen_random_uuid(), case_id uuid not null references public.cases(id) on delete cascade, organization_id uuid not null references public.organizations(id), type text not null, from_status public.case_status, to_status public.case_status, body text not null default '', customer_visible boolean not null default false, actor_user_id uuid not null references auth.users(id), created_at timestamptz not null default now());
create table public.quotes (id uuid primary key default gen_random_uuid(), case_id uuid not null references public.cases(id) on delete cascade, organization_id uuid not null references public.organizations(id), amount_paise bigint not null check (amount_paise >= 0), terms text not null, state text not null default 'pending', accepted_at timestamptz, created_at timestamptz not null default now());
create table public.credential_sets (id uuid primary key default gen_random_uuid(), case_id uuid not null references public.cases(id) on delete cascade, organization_id uuid not null references public.organizations(id), state text not null default 'not_submitted', ciphertext text, key_version text, created_at timestamptz not null default now(), expires_at timestamptz);
create table public.attachments (id uuid primary key default gen_random_uuid(), case_id uuid not null references public.cases(id) on delete cascade, organization_id uuid not null references public.organizations(id), kind text not null, storage_path text not null unique, content_type text not null, byte_size bigint not null check (byte_size > 0 and byte_size <= 26214400), customer_visible boolean not null default false, scan_state text not null default 'pending', created_at timestamptz not null default now());
...
create or replace function public.set_response_deadline() returns trigger language plpgsql as $$ begin if new.payment_verified_at is not null and new.access_usable_at is not null then new.response_deadline_at := greatest(new.payment_verified_at, new.access_usable_at) + interval '4 hours'; else new.response_deadline_at := null; end if; new.updated_at := now(); return new; end $$;
...
create or replace function public.prevent_event_update() returns trigger language plpgsql as $$ begin raise exception 'case events are append-only'; end $$;
...
create or replace function public.prevent_audit_update() returns trigger language plpgsql as $$ begin raise exception 'audit logs are append-only'; end $$;
...
create policy case_events_staff_insert on public.case_events for insert with check (public.is_staff() or (customer_visible and public.is_org_member(organization_id)));
...
-- Credential ciphertext is a Milestone 3 concern and is never readable through
-- the customer role; the future assigned-staff reveal action will use a short
-- lived server-side transaction with recent MFA.
create policy credentials_staff_only on public.credential_sets for select using (public.is_staff());
...
create policy audit_insert_authenticated on public.audit_logs for insert with check (auth.uid() = actor_user_id and (public.is_staff() or public.is_org_member(organization_id)));
